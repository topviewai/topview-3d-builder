...
| `topview-3d-cli studio open <dir>` | From a repository checkout, start `editor/apps/studio` with Node on `127.0.0.1:3002` when that port is free (`TOPVIEW3D_PROJECTS` set to this project) and return `url` for `/?project=<id>`. It does not open a browser. Fails with `STUDIO_UNAVAILABLE` outside a checkout, `STUDIO_PROJECT_MISSING` when Studio is already running without this project, or `STUDIO_START_FAILED` when it does not become ready. |
